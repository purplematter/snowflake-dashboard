const snowflakeDashboard = ($) => {
  ...
  // handle hydro deposits
  const depositHydro = async () => {
    let account = window.w3w.getAccount() // set account at the beginning to lock it in
    let amount = $('#snowflakeBalances input').val()
    var standardizedAmount = window.w3w.getWeb3js().utils.toWei(amount, 'ether') // * 1e18

    let method = hydroTokenContract.methods.approveAndCall(snowflakeAddress, standardizedAmount, '0x00')
    // estimate gas to make sure the transaction doesn't fail
    let estimatedGas = await method.estimateGas({ from: account })
      .catch(() => { return null })
    if (estimatedGas == null) {
      $('#snowflakeBalances div').html(`<p>Gas required exceeds allowance or the transaction always fails.</p>`)
      return
    }

    $('#snowflakeBalances div').html('<p>Awaiting Transaction Output...</p>')
    method.send({ from: account })
      .on('transactionHash', transactionHash => {
        $('#snowflakeBalances div').html(linkify('transaction', transactionHash))
      })
      .on('error', error => {
        $('#snowflakeBalances div').html(`<p>${error.toString().split('\n')[0]}</p>`)
      })
  }
  ...
  const w3wConfig = {
    handlers: {
      ...
      web3Ready: () => {
        ...
        hydroTokenContract = new web3js.eth.Contract(hydroTokenABI, hydroTokenAddress)
        $('#mintSnowflake').on('click', mintSnowflake)
        $('#snowflakeBalances button').on('click', depositHydro)
      ...
      web3AccountChangeHandler: (account, oldAccount) => {
        if (account === null) {
          ...
          clientRaindropContract.methods.getUserByAddress(account).call()
            .then(hydroId => {
              $('#hydroId').text(`HydroID: '${hydroId}'`)
            })
            .catch(() => {
              $('#hydroId').text('HydroID: Not Found')
            })
          snowflakeContract.methods.getHydroId(account).call()
            .then(hydroId => {
              $('#hasSnowflake').text('Has Snowflake ✅')
            })
            .catch(() => {
              $('#hasSnowflake').text('Has Snowflake ❌')
            })
          hydroTokenContract.methods.balances(account).call()
            .then(balance => {
              var standardized = window.w3w.getWeb3js().utils.fromWei(balance, 'ether')
              standardized = Number(standardized).toLocaleString(undefined, { maximumFractionDigits: 4 })
              $('#hydroBalance').text(`${standardized} 💧`)
            })
          snowflakeContract.methods.deposits(account).call()
            .then(balance => {
              var standardized = window.w3w.getWeb3js().utils.fromWei(balance, 'ether')
              standardized = Number(standardized).toLocaleString(undefined, { maximumFractionDigits: 4 })
              $('#snowflakeBalance').text(`${standardized} 💧`)
            })
        }
      }
    },
